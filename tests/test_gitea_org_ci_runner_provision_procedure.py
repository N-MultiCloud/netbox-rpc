...
MIGRATION_MODULE = "netbox_rpc.migrations.0080_seed_gitea_org_ci_runner_provision"
PROCEDURE_ID = "service.gitea.actions_runner.provision_org_ci_runner"
RUNNER_LABEL = "ci-untrusted-python312"
...
def test_params_schema_accepts_only_bounded_runner_inputs(migration) -> None:
    ...
    validate({"registration_token_secret_ref": SECRET_REF}, schema)
    validate(
        {
    ...
    for invalid in (
        ...
        {"registration_token": "plain-token"},
        ...
        {"build_runner_image": True, "load_prebuilt_runner_image": True},
...
def test_result_schema_requires_completed_state_for_success(migration) -> None:
    ...
    success = {
        ...
        "runner_labels": [f"{RUNNER_LABEL}:docker://{RUNNER_IMAGE}"],
        "gitea_instance_url": "http://10.0.30.96:3000",
        "compose_project_dir": "/opt/gitea-ci-runner",
...
def test_normalizer_emits_fixed_runner_contract_when_gate_is_open(
    ...
    assert normalized["runner_name"] == "ci-untrusted-nmulticloud-org-241"
    assert normalized["runner_label"] == RUNNER_LABEL
    assert normalized["runner_labels"] == [f"{RUNNER_LABEL}:docker://{RUNNER_IMAGE}"]
    assert normalized["runner_image"] == RUNNER_IMAGE
    assert normalized["compose_project_dir"] == "/opt/gitea-ci-runner"
...
@pytest.mark.parametrize(
    ...
    [
        ({}, "RPC_PARAM_INVALID"),
        ({"registration_token_secret_ref": "plain-token"}, "RPC_PARAM_INVALID"),
        (
            {"registration_token_secret_ref": SECRET_REF, "runner_name": "ci runner"},
            "RPC_PARAM_INVALID",
        ),
        (
            {
        ...
        (
            {"registration_token_secret_ref": SECRET_REF, "runner_labels": ["prod"]},
            "RPC_PARAM_INVALID",
        ),
        (
            {"registration_token_secret_ref": SECRET_REF, "rpc_ssh_host": "10.0.30.99"},
            "RPC_PARAM_INVALID",
        ),
    ],
)
...
def test_normalizer_requires_supported_assigned_object(
    ...
    with pytest.raises(normalization_module.RPCExecutionError) as excinfo:
        normalization_module.normalize_execution_params(
            _execution({"registration_token_secret_ref": SECRET_REF}, **kwargs)
        )
...
class _FakeCommandManager:
    ...
    def update_or_create(self, *, procedure: _FakeProcedure, sequence: int, defaults: dict):
