from __future__ import annotations

import hashlib
import importlib
import json
import runpy
import sys
import types
from copy import deepcopy
from pathlib import Path
from types import SimpleNamespace

import pytest
from jsonschema import ValidationError, validate

ROOT = Path(__file__).resolve().parents[1]
MIGRATION_MODULE = "netbox_rpc.migrations.0091_seed_gitea_docker_runner_recovery"
DIAGNOSE = "service.gitea.actions_runner.diagnose_user_ci_runner"
RECOVER = "service.gitea.actions_runner.recover_user_ci_runner"
EXPECTED_HASHES = {
    DIAGNOSE: "8e9a33d189f2850e95a77965505cb7e2625ba20c642fc22c7326812bdadb9e7f",
    RECOVER: "4597aaf1d59c1fc554a3993316ea85444e4673801ea133ebc04c33fcd375da20",
}
CONTRACT = runpy.run_path(str(ROOT / "netbox_rpc/gitea_docker_runner_contract.py"))


@pytest.fixture()
def migration(monkeypatch: pytest.MonkeyPatch):
    netbox = types.ModuleType("netbox")
    netbox_plugins = types.ModuleType("netbox.plugins")
    netbox_plugins.PluginConfig = type("PluginConfig", (), {"ready": lambda self: None})
    django = types.ModuleType("django")
    django_db = types.ModuleType("django.db")
    django_migrations = types.ModuleType("django.db.migrations")
    django_migrations.Migration = type("Migration", (), {})
    django_migrations.RunPython = lambda *args, **kwargs: (args, kwargs)
    django_db.migrations = django_migrations
    django.db = django_db
    monkeypatch.setitem(sys.modules, "netbox", netbox)
    monkeypatch.setitem(sys.modules, "netbox.plugins", netbox_plugins)
    monkeypatch.setitem(sys.modules, "django", django)
    monkeypatch.setitem(sys.modules, "django.db", django_db)
    monkeypatch.setitem(sys.modules, "django.db.migrations", django_migrations)
    sys.modules.pop(MIGRATION_MODULE, None)
    module = importlib.import_module(MIGRATION_MODULE)
    yield module
    sys.modules.pop(MIGRATION_MODULE, None)


def test_exact_schema_and_capability_hashes_match_backend() -> None:
    assert CONTRACT["PARAMS_SCHEMA_SHA256"] == (
        "f59454b0027f98b98b7c7e0ce1cc767fb8bd7c33aecbfb9a0910e6d21a46de34"
    )
    assert CONTRACT["DIAGNOSE_RESULT_SCHEMA_SHA256"] == (
        "b6a44632445e97d41a84502646fcd57627359aa4fcf09dfbf7b9d564cadb18b6"
    )
    assert CONTRACT["RECOVER_RESULT_SCHEMA_SHA256"] == (
        "30610aa05dfcb25f8f44e017cb942092e39f56ec1bf5ec59bebae5bb1319ccb1"
    )
    assert CONTRACT["CAPABILITY_CONTRACT_SHA256"] == EXPECTED_HASHES


def test_omitted_recovery_capability_is_not_ready_for_admission() -> None:
    capability_module = runpy.run_path(str(ROOT / "netbox_rpc/capabilities.py"))
    manifest = capability_module["BackendCapabilityManifest"](
        envelope_version=1,
        handlers=[
            {
                "handler_id": DIAGNOSE,
                "version": 1,
                "effect": "read",
                "contract_hash": EXPECTED_HASHES[DIAGNOSE],
            }
        ],
    )
    recovery = SimpleNamespace(handler_id=RECOVER, version=1, effect="write")

    assert (
        capability_module["verify_procedure_capability"](recovery, manifest)
        is capability_module["CapabilityStatus"].MISMATCH
    )

    constants = runpy.run_path(str(ROOT / "netbox_rpc/constants.py"))
    assert RECOVER in constants["EXPLICIT_BACKEND_CAPABILITY_PROCEDURE_NAMES"]


def test_empty_params_and_literal_fields_reject_near_misses() -> None:
    validate({}, CONTRACT["PARAMS_SCHEMA"])
    with pytest.raises(ValidationError):
        validate({"host": "runner"}, CONTRACT["PARAMS_SCHEMA"])

    valid = _diagnose_result()
    validate(valid, CONTRACT["DIAGNOSE_RESULT_SCHEMA"])
    for key, near_miss in (
        ("procedure", DIAGNOSE.replace(".", "x", 1)),
        ("target", "GiteaxRunner"),
        ("target_object_id", 416),
        ("lane", "userxubuntu"),
    ):
        mutated = deepcopy(valid)
        mutated[key] = near_miss
        with pytest.raises(ValidationError):
            validate(mutated, CONTRACT["DIAGNOSE_RESULT_SCHEMA"])


def test_recovery_network_ids_are_item_bounded() -> None:
    result = {
        "ok": False,
        "procedure": RECOVER,
        "target": "Gitea-Runner",
        "target_object_id": 604,
        "lane": "user-ubuntu",
        "stage": "refused_active_job",
        "before": _snapshot(active_job=True),
        "after": None,
        "removed_network_ids": [],
        "resolver_reconciled": False,
        "restart_performed": False,
        "refused_active_job": True,
    }
    validate(result, CONTRACT["RECOVER_RESULT_SCHEMA"])
    result["removed_network_ids"] = ["../attacker"]
    with pytest.raises(ValidationError):
        validate(result, CONTRACT["RECOVER_RESULT_SCHEMA"])


def test_migration_seeds_both_disabled_exact_rows_and_commands(migration) -> None:
    procedures = _ProcedureManager()
    commands = _CommandManager()
    migration.seed_gitea_docker_runner_recovery(_apps(procedures, commands), None)

    assert set(procedures.rows) == {DIAGNOSE, RECOVER}
    assert procedures.rows[DIAGNOSE]["enabled"] is False
    assert procedures.rows[DIAGNOSE]["effect"] == "read"
    assert procedures.rows[DIAGNOSE]["approval_required"] is False
    assert procedures.rows[RECOVER]["enabled"] is False
    assert procedures.rows[RECOVER]["effect"] == "write"
    assert procedures.rows[RECOVER]["approval_required"] is True
    assert procedures.rows[DIAGNOSE]["params_schema"] == CONTRACT["PARAMS_SCHEMA"]
    assert (
        procedures.rows[DIAGNOSE]["result_schema"] == CONTRACT["DIAGNOSE_RESULT_SCHEMA"]
    )
    assert (
        procedures.rows[RECOVER]["result_schema"] == CONTRACT["RECOVER_RESULT_SCHEMA"]
    )
    assert commands.rows[(DIAGNOSE, 1)]["argv"] == [
        "backend-orchestrated",
        "gitea-user-ci-runner-diagnose",
    ]
    assert commands.rows[(RECOVER, 1)]["argv"] == [
        "backend-orchestrated",
        "gitea-user-ci-runner-recover",
    ]
    assert migration.Migration.dependencies == [
        ("netbox_rpc", "0090_execution_credential_authority")
    ]


def test_migration_capability_payloads_have_exact_backend_hashes(migration) -> None:
    for seed in migration._PROCEDURES:
        name = seed["name"]
        effect = seed["defaults"]["effect"]
        command = {"sequence": 1, **commands_without_description(seed)}
        payload = {
            "handler_id": name,
            "version": 1,
            "effect": effect,
            "commands": [command],
            "semantic_contract": CONTRACT["SEMANTIC_CONTRACTS"][name],
        }
        assert _sha256(payload) == EXPECTED_HASHES[name]


def test_reverse_disables_without_deleting_or_touching_unrelated_rows(
    migration,
) -> None:
    procedures = _ProcedureManager()
    commands = _CommandManager()
    migration.seed_gitea_docker_runner_recovery(_apps(procedures, commands), None)
    procedures.rows["unrelated"] = {"enabled": True}

    migration.disable_gitea_docker_runner_recovery(_apps(procedures, commands), None)

    assert procedures.rows[DIAGNOSE]["enabled"] is False
    assert procedures.rows[RECOVER]["enabled"] is False
    assert procedures.rows["unrelated"]["enabled"] is True
    assert procedures.deleted == []


def test_security_and_gate_wiring_is_explicit() -> None:
    constants = runpy.run_path(str(ROOT / "netbox_rpc/constants.py"))
    assert {DIAGNOSE, RECOVER} <= constants[
        "EXPLICIT_BACKEND_CAPABILITY_PROCEDURE_NAMES"
    ]
    assert RECOVER in constants["PROTECTED_APPROVAL_PROCEDURE_NAMES"]
    assert DIAGNOSE not in constants["PROTECTED_APPROVAL_PROCEDURE_NAMES"]
    assert {DIAGNOSE, RECOVER} <= constants["GITEA_USER_CI_RUNNER_PROCEDURE_NAMES"]
    normalizer = (ROOT / "netbox_rpc/domain/normalization.py").read_text()
    handlers = (ROOT / "netbox_rpc/application/command_handlers.py").read_text()
    jobs = (ROOT / "netbox_rpc/jobs.py").read_text()
    assert "validate_gitea_docker_runner_target" in normalizer
    assert "credentials are resolved from the assigned target" in normalizer
    assert "GITEA_USER_CI_RUNNER_PROCEDURE_NAMES" in handlers
    assert "GITEA_USER_CI_RUNNER_RECOVER: gitea_docker_runner_contract" in handlers
    assert "BACKEND_RESPONSE_MAX_BYTES" in jobs


def commands_without_description(seed: dict[str, object]) -> dict[str, object]:
    return {
        "step_type": "shell_argv",
        "device_cli_mode": "",
        "argv": seed["argv"],
        "render_mode": "literal",
        "produces_var": "",
        "capture_kind": "",
        "capture_expression": "",
        "condition_param": "",
        "condition_negate": False,
        "for_each_param": "",
        "continue_on_error": False,
    }


def _sha256(value: object) -> str:
    return hashlib.sha256(
        json.dumps(value, sort_keys=True, separators=(",", ":"), default=str).encode()
    ).hexdigest()


def _snapshot(*, active_job: bool = False) -> dict[str, object]:
    return {
        "docker_active": True,
        "runner_container_name": "ci-ubuntu-emersonfelipesp-241",
        "runner_container_id": "a" * 12,
        "runner_state": "running",
        "active_job": active_job,
        "daemon_dns": ["168.0.96.26", "168.0.96.27"],
        "default_address_pools": ["base=172.30.0.0/16,size=24"],
        "probes": [
            {
                "host": host,
                "resolved": True,
                "addresses": ["10.0.30.96"],
                "status": "ok",
            }
            for host in ("git.nmulti.cloud", "github.com")
        ],
        "networks": [
            {
                "id": "b" * 12,
                "name": "GITEA-ACTIONS-TASK-1",
                "attached_containers": 0,
                "disposition": "stale",
            }
        ],
        "address_pool_exhausted": False,
        "last_log_activity": None,
        "truncated": False,
    }


def _diagnose_result() -> dict[str, object]:
    return {
        **_snapshot(),
        "ok": True,
        "procedure": DIAGNOSE,
        "target": "Gitea-Runner",
        "target_object_id": 604,
        "lane": "user-ubuntu",
        "stage": "complete",
    }


def _apps(procedures, commands):
    def get_model(app_label: str, model_name: str):
        if (app_label, model_name) == ("netbox_rpc", "RPCProcedure"):
            return SimpleNamespace(objects=procedures)
        if (app_label, model_name) == ("netbox_rpc", "RPCProcedureCommand"):
            return SimpleNamespace(objects=commands)
        raise AssertionError((app_label, model_name))

    return SimpleNamespace(get_model=get_model)


class _Query:
    def __init__(self, manager: _ProcedureManager, names: set[str]) -> None:
        self.manager = manager
        self.names = names

    def exists(self) -> bool:
        return any(name in self.manager.rows for name in self.names)

    def update(self, **fields: object) -> int:
        matches = [name for name in self.names if name in self.manager.rows]
        for name in matches:
            self.manager.rows[name].update(fields)
        return len(matches)


class _ProcedureManager:
    def __init__(self) -> None:
        self.rows: dict[str, dict[str, object]] = {}
        self.deleted: list[str] = []

    def filter(self, *, name=None, name__in=None):
        names = {name} if name is not None else set(name__in or ())
        return _Query(self, names)

    def create(self, *, name: str, **fields: object):
        self.rows[name] = dict(fields)
        return SimpleNamespace(name=name, handler_id=fields["handler_id"])


class _CommandManager:
    def __init__(self) -> None:
        self.rows: dict[tuple[str, int], dict[str, object]] = {}

    def create(self, *, procedure, sequence: int, **fields: object):
        self.rows[(procedure.name, sequence)] = dict(fields)
        return SimpleNamespace()
