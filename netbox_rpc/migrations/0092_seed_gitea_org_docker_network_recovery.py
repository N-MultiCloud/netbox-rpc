"""Seed fixed organization-runner Docker network diagnosis and recovery."""

from django.db import migrations

_DIAGNOSE_NAME = "service.gitea.actions_runner.diagnose_org_ci_runner"
_RECOVER_NAME = "service.gitea.actions_runner.recover_org_ci_runner"
...
_DIAGNOSE_RESULT_SCHEMA = {
    "$defs": {"NetworkEvidence": _NETWORK_SCHEMA},
    "additionalProperties": False,
    "properties": {
        **_snapshot_properties(),
        ...
        "lane": {"const": "general-ubuntu", "title": "Lane", "type": "string"},
        "stage": {
...
_PROCEDURES = (
    {
        ...
        "defaults": {
            **_procedure_defaults(
            ...
            "description": (
                "Diagnose Docker address-pool and task-network state for the fixed "
                "organization CI runner with no caller-selected inputs."
            ),
        },
        "argv": ["backend-orchestrated", "gitea-org-ci-runner-diagnose"],
        "command_description": (
    ...
    },
    {
        ...
        "defaults": {
            **_procedure_defaults(
            ...
            "description": (
                "Approval-gated reclamation of proven-idle task networks for the "
                "fixed organization CI runner; accepts no caller selectors."
            ),
        },
        "argv": ["backend-orchestrated", "gitea-org-ci-runner-recover"],
        "command_description": (
...
def seed_gitea_org_docker_network_recovery(apps, schema_editor):
    ...
    for seed in _PROCEDURES:
        if RPCProcedure.objects.filter(name=seed["name"]).exists():
            raise RuntimeError(
                "Migration 0092 cannot adopt an existing organization runner "
...
def disable_gitea_org_docker_network_recovery(apps, schema_editor):
    RPCProcedure = apps.get_model("netbox_rpc", "RPCProcedure")
...
class Migration(migrations.Migration):
    dependencies = [  # noqa: RUF012
        ("netbox_rpc", "0091_seed_gitea_docker_runner_recovery")
    ]

    operations = [  # noqa: RUF012
        migrations.RunPython(
            seed_gitea_org_docker_network_recovery,
            reverse_code=disable_gitea_org_docker_network_recovery,
        )
    ]
